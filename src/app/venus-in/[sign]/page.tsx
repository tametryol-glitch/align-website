...
import { ALL_SIGN_KEYS, SIGNS, getElementColor, getVenusSignContent, VENUS_SYMBOL, type ZodiacSign } from '@/data/venusSignContent';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `Venus in ${s.name} — Love Language, Attraction & Relationship Style`;
  const description = `Discover what Venus in ${s.name} (${s.glyph}) means for your love language, aesthetic taste, attraction style, and relationships. Deep astrological profile.`;

  return {
    title, description,
    keywords: [`venus in ${s.name.toLowerCase()}`, `${s.name.toLowerCase()} venus`, 'venus sign', 'love astrology', `${s.element} venus`, 'relationship astrology'],
    openGraph: { title: `${title} | Align`, description, url: `https://aligncosmic.com/venus-in/${sign}`, siteName: 'Align', type: 'article' },
    twitter: { card: 'summary_large_image', title: `${title} | Align`, description },
    alternates: { canonical: `https://aligncosmic.com/venus-in/${sign}` },
  };
}

export default async function VenusSignPage({ params }: PageProps) {
  const { sign } = await params;
  if (!ALL_SIGN_KEYS.includes(sign as ZodiacSign)) notFound();
  const content = getVenusSignContent(sign as ZodiacSign);
  const s = SIGNS[sign as ZodiacSign];
  const elementColor = getElementColor(s.element);

  const jsonLd = {
    '@context': 'https://schema.org', '@type': 'Article',
    headline: content.headline, description: content.intro.slice(0, 160),
    publisher: { '@type': 'Organization', name: 'Align', url: 'https://aligncosmic.com' },
    mainEntityOfPage: `https://aligncosmic.com/venus-in/${sign}`,
  };
    ...
    <div className="min-h-screen">
      ...
      <nav className="flex items-center justify-between px-6 py-4 max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          <Link href="/venus-in" className="text-sm text-text-secondary hover:text-text-primary transition-colors">All Venus Signs</Link>
      ...
      <div className="max-w-4xl mx-auto px-6 pt-2 pb-4">
        <nav className="text-sm text-text-muted" aria-label="Breadcrumb">
          <ol className="flex items-center gap-1.5">
            ...
            <li><Link href="/venus-in" className="hover:text-text-secondary transition-colors">Venus Signs</Link></li>
            <li className="text-text-muted">/</li>
            <li className="text-text-tertiary">Venus in {s.name}</li>
          </ol>
        </nav>
      </div>
      ...
      <header className="max-w-4xl mx-auto px-6 pt-8 pb-16 text-center">
        ...
        <h1 className="text-3xl sm:text-5xl font-display font-bold text-text-primary leading-tight mb-4">
          Venus in {s.name}
          <br /><span className="text-2xl sm:text-3xl text-text-secondary font-normal">Your Love Language</span>
        </h1>
        <div className="flex items-center justify-center gap-4 flex-wrap mt-4">
          <span className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-medium border" style={{ borderColor: elementColor, color: elementColor, backgroundColor: `${elementColor}18` }}>
            {s.element.charAt(0).toUpperCase() + s.element.slice(1)} Venus
          </span>
        ...
        {content.sections.map((section) => (
          <article key={section.title} className="scroll-mt-24" id={section.title.toLowerCase().replace(/[^a-z0-9]+/g, '-')}>
            ...
            <div className="mt-10 flex items-center gap-4">
              <div className="flex-1 h-px bg-border-primary" /><span className="text-text-muted text-xs">&#10022;</span><div className="flex-1 h-px bg-border-primary" />
            </div>
          </article>
        ))}
      </main>
      ...
      <section className="max-w-4xl mx-auto px-6 pb-12">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {[
            ...
            { href: `/mercury-in/${sign}`, label: 'Mercury Sign', text: `Mercury in ${s.name}` },
      ...
      <section className="max-w-4xl mx-auto px-6 pb-20">
        <div className="bg-gradient-cosmic rounded-3xl p-8 sm:p-12 border border-accent-muted text-center">
          ...
          <p className="text-text-tertiary max-w-lg mx-auto mb-8">Venus is just one piece of your love story. Discover your complete relationship profile with Align.</p>
      ...
      <footer className="border-t border-border-primary py-8 px-6">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-6 text-sm text-text-muted">
            <Link href="/venus-in" className="hover:text-text-secondary">All Venus Signs</Link>
