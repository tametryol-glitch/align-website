...
/**
 * One photo in the profile gallery. `target` is what reactions key on:
 * post photos react against the post (shared with the feed), profile
 * photos against photo_reactions.
 */
...
export default function UserProfilePage() {
  ...
  // Photo viewer — click any profile photo to open it full-screen and react.
  ...
  async function loadPhotos() {
    ...
    try {
      ...
      try {
        setPhotoReactions(await batchGetPhotoReactions(merged.map(m => m.target), user?.id || null));
  ...
  /** Open a header photo (avatar / cover), loading the gallery if needed. */
  function openHeaderPhoto(url: string | null, label: string) {
    if (!url) return;
