...
export const metadata: Metadata = {
  ...
  description:
    'Explore all 12 Moon signs with in-depth guides on emotional needs, inner world, and what each Moon sign needs in love. Discover your lunar placement.',
  keywords: [
    'moon sign', 'moon signs', 'moon sign meaning', 'what is my moon sign',
    'moon in aries', 'moon in taurus', 'moon in gemini', 'moon in cancer',
    'moon in leo', 'moon in virgo', 'moon in libra', 'moon in scorpio',
    'moon in sagittarius', 'moon in capricorn', 'moon in aquarius', 'moon in pisces',
    'lunar astrology', 'emotional astrology',
  ],
  openGraph: {
    title: 'Moon Signs — Your Emotional Blueprint | Align', description: 'In-depth guides for every Moon sign.',
...
const ELEMENT_GROUPS: { element: 'fire' | 'earth' | 'air' | 'water'; label: string; signs: ZodiacSign[] }[] = [
  { element: 'fire', label: 'Fire Moon Signs', signs: ['aries', 'leo', 'sagittarius'] },
  { element: 'earth', label: 'Earth Moon Signs', signs: ['taurus', 'virgo', 'capricorn'] },
  { element: 'air', label: 'Air Moon Signs', signs: ['gemini', 'libra', 'aquarius'] },
  { element: 'water', label: 'Water Moon Signs', signs: ['cancer', 'scorpio', 'pisces'] },
];

export default function MoonSignIndexPage() {
  const jsonLd = {
    '@context': 'https://schema.org', '@type': 'CollectionPage',
    name: 'Moon Signs — All 12 Lunar Placements',
    description: 'Complete guide to all 12 Moon signs.',
    publisher: { '@type': 'Organization', name: 'Align', url: 'https://aligncosmic.com' },
    mainEntityOfPage: 'https://aligncosmic.com/moon-sign',
  };
    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-5xl mx-auto px-6 pt-2 pb-4">
        <nav className="text-sm text-text-muted" aria-label="Breadcrumb">
          <ol className="flex items-center gap-1.5">
            ...
            <li className="text-text-tertiary">Moon Signs</li>
          </ol>
        </nav>
      </div>
      ...
      <header className="max-w-5xl mx-auto px-6 pt-8 pb-16 text-center">
        <div className="flex items-center justify-center gap-2 mb-6 text-3xl">
          {ALL_SIGN_KEYS.map((sk) => (<span key={sk} className="opacity-60">{SIGNS[sk].glyph}</span>))}
        </div>
        <h1 className="text-3xl sm:text-5xl font-display font-bold text-text-primary leading-tight mb-4">Moon Signs</h1>
        <p className="text-xl sm:text-2xl font-display text-text-secondary mb-2">Your Emotional Blueprint</p>
        <p className="text-text-tertiary max-w-2xl mx-auto text-base sm:text-lg">
          Your Moon sign reveals your emotional core — how you process feelings, what you need for security,
          and how you love at the deepest level. Explore all 12 lunar placements.
        </p>
      </header>
            ...
            <section key={group.element}>
              ...
              <div className="grid sm:grid-cols-3 gap-4">
                {group.signs.map((signKey) => {
                  ...
                  return (
                    <Link key={signKey} href={`/moon-sign/${signKey}`} className="bg-bg-card border border-border-primary rounded-2xl p-6 hover:border-accent-primary/30 transition-all group relative overflow-hidden">
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="min-w-0">
                          <h3 className="text-lg font-display font-semibold text-text-primary group-hover:text-accent-primary transition-colors">Moon in {sd.name}</h3>
                          <p className="text-xs text-text-muted mb-2">{sd.element.charAt(0).toUpperCase() + sd.element.slice(1)} Moon</p>
                        </div>
                      </div>
                    </Link>
                  );
      ...
      <section className="max-w-4xl mx-auto px-6 pb-20">
        <div className="bg-gradient-cosmic rounded-3xl p-8 sm:p-12 border border-accent-muted text-center">
          <h2 className="text-2xl sm:text-3xl font-display font-bold text-text-primary mb-4">Discover Your Moon Sign</h2>
          <p className="text-text-tertiary max-w-lg mx-auto mb-8">
            Your Moon sign is calculated from your exact birth time and location. Align generates your complete
            natal chart with AI-powered interpretations.
          </p>
