...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `Moon in ${s.name} — Emotional Needs, Inner World & Love`;
  const description = `Discover what Moon in ${s.name} (${s.glyph}) means for your emotions, inner world, and what you need in love. Deep dive into your lunar placement.`;

  return {
    title,
    description,
    keywords: [
      `moon in ${s.name.toLowerCase()}`, `${s.name.toLowerCase()} moon`, `${s.name.toLowerCase()} moon sign`,
      'moon sign', 'lunar placement', 'emotional astrology', `${s.element} moon`, 'astrology',
    ],
    openGraph: {
      title: `${title} | Align`, description,
...
export default async function MoonSignPage({ params }: PageProps) {
  ...
  const content = getMoonSignContent(sign as ZodiacSign);
  const s = SIGNS[sign as ZodiacSign];
  ...
  const jsonLd = {
    ...
    mainEntityOfPage: `https://aligncosmic.com/moon-sign/${sign}`,
  };
    ...
    <div className="min-h-screen">
      ...
      <nav className="flex items-center justify-between px-6 py-4 max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          <Link href="/moon-sign" className="text-sm text-text-secondary hover:text-text-primary transition-colors">All Moon Signs</Link>
      ...
      <div className="max-w-4xl mx-auto px-6 pt-2 pb-4">
        <nav className="text-sm text-text-muted" aria-label="Breadcrumb">
          <ol className="flex items-center gap-1.5">
            ...
            <li><Link href="/moon-sign" className="hover:text-text-secondary transition-colors">Moon Signs</Link></li>
            <li className="text-text-muted">/</li>
            <li className="text-text-tertiary">Moon in {s.name}</li>
          </ol>
        </nav>
      </div>
      ...
      <header className="max-w-4xl mx-auto px-6 pt-8 pb-16 text-center">
        ...
        <h1 className="text-3xl sm:text-5xl font-display font-bold text-text-primary leading-tight mb-4">
          Moon in {s.name}
          <br />
          <span className="text-2xl sm:text-3xl text-text-secondary font-normal">Your Emotional Blueprint</span>
        </h1>
        ...
        <div className="flex items-center justify-center gap-4 flex-wrap mt-4">
          <span className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-medium border" style={{ borderColor: elementColor, color: elementColor, backgroundColor: `${elementColor}18` }}>
            {s.element.charAt(0).toUpperCase() + s.element.slice(1)} Moon
          </span>
          <span className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-medium border border-border-primary text-text-secondary bg-bg-card">
            {s.modality.charAt(0).toUpperCase() + s.modality.slice(1)} Quality
          </span>
        </div>
      </header>
      ...
      <section className="max-w-4xl mx-auto px-6 pb-12">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          ...
          <Link href={`/rising-sign/${sign}`} className="bg-bg-card border border-border-primary rounded-xl p-4 text-center hover:border-accent-primary/30 transition-colors group">
            <p className="text-xs text-text-muted mb-1">Rising Sign</p>
            <p className="text-sm font-medium text-text-primary group-hover:text-accent-primary transition-colors">{s.name} Rising</p>
          </Link>
      ...
      <section className="max-w-4xl mx-auto px-6 pb-20">
        <div className="bg-gradient-cosmic rounded-3xl p-8 sm:p-12 border border-accent-muted text-center">
          ...
          <p className="text-text-tertiary max-w-lg mx-auto mb-8">
            Your Moon sign is just one layer of your emotional landscape. Discover your complete cosmic blueprint with Align.
          </p>
      ...
      <footer className="border-t border-border-primary py-8 px-6">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-6 text-sm text-text-muted">
            <Link href="/moon-sign" className="hover:text-text-secondary">All Moon Signs</Link>
