...
import { ALL_SIGN_KEYS, SIGNS, getElementColor, getChironSignContent, CHIRON_SYMBOL, type ZodiacSign } from '@/data/chironSignContent';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `Chiron in ${s.name} — Healing, Wounds & Wisdom`;
  const description = `Discover what Chiron in ${s.name} (${s.glyph}) means for your healing and wisdom. In-depth astrological profile.`;
  return {
    title, description,
    keywords: [`chiron in ${s.name.toLowerCase()}`, `${s.name.toLowerCase()} chiron`, 'chiron sign', `${s.element} chiron`],
    openGraph: { title: `${title} | Align`, description, url: `https://aligncosmic.com/chiron-in/${sign}`, siteName: 'Align', type: 'article' },
    twitter: { card: 'summary_large_image', title: `${title} | Align`, description },
    alternates: { canonical: `https://aligncosmic.com/chiron-in/${sign}` },
  };
}

export default async function ChironSignPage({ params }: PageProps) {
  const { sign } = await params;
  if (!ALL_SIGN_KEYS.includes(sign as ZodiacSign)) notFound();
  const content = getChironSignContent(sign as ZodiacSign);
  const s = SIGNS[sign as ZodiacSign];
  ...
  const jsonLd = {
    ...
    mainEntityOfPage: `https://aligncosmic.com/chiron-in/${sign}`,
  };
    ...
    <div className="min-h-screen">
      ...
      <nav className="flex items-center justify-between px-6 py-4 max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          <Link href="/chiron-in" className="text-sm text-text-secondary hover:text-text-primary transition-colors">All Chiron Signs</Link>
      ...
      <div className="max-w-4xl mx-auto px-6 pt-2 pb-4">
        <nav className="text-sm text-text-muted" aria-label="Breadcrumb">
          <ol className="flex items-center gap-1.5">
            ...
            <li><Link href="/chiron-in" className="hover:text-text-secondary transition-colors">Chiron Signs</Link></li>
            <li className="text-text-muted">/</li>
            <li className="text-text-tertiary">Chiron in {s.name}</li>
          </ol>
        </nav>
      </div>
      <header className="max-w-4xl mx-auto px-6 pt-8 pb-16 text-center">
        <div className="flex items-center justify-center mb-8">
          <div className="relative">
            ...
            <span className="absolute -bottom-2 left-1/2 -translate-x-1/2 text-xs font-medium px-3 py-1 rounded-full border" style={{ borderColor: elementColor, color: elementColor, backgroundColor: `${elementColor}18` }}>
              {CHIRON_SYMBOL} Chiron
            </span>
          </div>
        </div>
        <h1 className="text-3xl sm:text-5xl font-display font-bold text-text-primary leading-tight mb-4">
          Chiron in {s.name}<br /><span className="text-2xl sm:text-3xl text-text-secondary font-normal">{content.subtitle}</span>
        </h1>
        <div className="flex items-center justify-center gap-4 flex-wrap mt-4">
          <span className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-medium border" style={{ borderColor: elementColor, color: elementColor, backgroundColor: `${elementColor}18` }}>
            {s.element.charAt(0).toUpperCase() + s.element.slice(1)} Chiron
          </span>
      ...
      <section className="max-w-4xl mx-auto px-6 pb-20">
        <div className="bg-gradient-cosmic rounded-3xl p-8 sm:p-12 border border-accent-muted text-center">
          ...
          <p className="text-text-tertiary max-w-lg mx-auto mb-8">Chiron reveals your healing and wisdom. Discover your complete cosmic blueprint with Align.</p>
      ...
      <footer className="border-t border-border-primary py-8 px-6">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-6 text-sm text-text-muted">
            <Link href="/chiron-in" className="hover:text-text-secondary">All Chiron Signs</Link>
