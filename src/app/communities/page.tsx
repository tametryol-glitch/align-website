  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="card rounded-2xl p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-text-primary">{t('communities.title')}</h1>
              <p className="text-sm text-text-secondary">{t('communities.subtitle')}</p>
            </div>
          </div>
          <Link
      ...
      <div className="flex gap-2">
        ...
        >
          {t('communities.tabs.myCommunities')} {myCommunities.length > 0 && `(${myCommunities.length})`}
        </button>
      </div>
        ...
        <>
          ...
          {loading ? (
            ...
            <div className="card rounded-2xl p-12 text-center">
              ...
              <h2 className="text-lg font-semibold text-text-primary mb-2">{t('communities.empty.title')}</h2>
              <p className="text-text-secondary mb-6">{t('communities.empty.description')}</p>
              <Link
                href="/communities/create"
                className="btn-primary inline-flex items-center gap-2 px-6 py-3 rounded-xl font-medium"
              >
                <Plus className="w-4 h-4" /> {t('communities.empty.createButton')}
              </Link>
            </div>
          ) : (
            <>
              ...
              {!search && !selectedCategory && featured.length > 0 && (
                <div>
                  <h2 className="text-sm font-semibold text-text-muted uppercase tracking-wider mb-3">{t('communities.featured')}</h2>
              ...
              {/* All Communities */}
              <div>
                {!search && !selectedCategory && <h2 className="text-sm font-semibold text-text-muted uppercase tracking-wider mb-3">{t('communities.allCommunities')}</h2>}
                  ...
                  {(search || selectedCategory ? allCommunities : discoverList).length === 0 && (
                    <div className="card rounded-2xl p-8 text-center">
                      <p className="text-text-muted text-sm">
                        {search ? t('communities.noResultsSearch', { query: search }) : t('communities.noResultsCategory')}
                      </p>
                    </div>
                  )}
                </div>
      ...
      {activeTab === 'mine' && (
        <>
          {loading ? (
            ...
            <div className="card rounded-2xl p-12 text-center">
              <div className="text-5xl mb-4">🌌</div>
              <h2 className="text-lg font-semibold text-text-primary mb-2">{t('communities.yourCircle.title')}</h2>
              <p className="text-text-secondary mb-6">{t('communities.yourCircle.description')}</p>
              <button
                onClick={() => setActiveTab('discover')}
                className="btn-primary inline-flex items-center gap-2 px-6 py-3 rounded-xl font-medium"
              >
                {t('communities.yourCircle.exploreButton')}
              </button>
            </div>
          ) : (
            <div className="space-y-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-2 flex-wrap">
            <span className="text-[11px] text-text-muted flex items-center gap-1">
              <Users className="w-3 h-3" /> {t('communities.memberCount', { count: comm.member_count })}
            </span>
            ...
            {!comm.is_public && (
              <span className="inline-flex items-center gap-1 text-[11px] text-text-muted">
                <Lock className="w-3 h-3" /> {t('communities.private')}
              </span>
            )}
            {comm.zodiac_sign && (
