  ...
  async function loadProfile(userId: string) {
    try {
      ...
      const { data } = await supabase
        .from('profiles')
        .select('id, display_name, username, email, avatar_url, cover_photo_url, bio, sun_sign, moon_sign, rising_sign, starseed, human_design_type, birth_date, birth_time, birth_location, latitude, longitude, timezone, is_subscribed, is_admin')
        .eq('id', userId)
        .single();
