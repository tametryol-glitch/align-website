...
export async function middleware(request: NextRequest) {
  ...
  const PUBLIC_API_ROUTES = ['/api/og', '/api/admin', '/api/cron', '/api/stripe', '/api/tts', '/api/transcribe', '/api/track', '/api/tiktok-oembed', '/api/meta-preview', '/api/aasa', '/api/push'];
  ...
  const isPublicPage = pathname === '/' ||
    ...
    pathname.startsWith('/support') ||   // App Store Connect requires a reachable Support URL; a login gate here fails review
...
export const config = {
  matcher: ['/((?!_next/static|_next/image|favicon.ico|.*\\.(?:svg|png|jpg|jpeg|gif|webp|mp4|webm|mov|mp3|wav|m4a|ogg|wasm|tflite|task)$).*)'],
};
